...
<div class="panel">
    ...
    <ul class="nav nav-tabs">
        <shiro:hasPermission name="activities:adpush:create">
        <c:if test="${op eq '新增'}">
            <li ${op eq '新增' ? 'class="active"' : ''}>
                <a href="${ctx}/activities/adpush?BackURL=<es:BackURL/>">
   	...
   	<c:if test="${not empty m.id}">
            <li ${op eq '查看' ? 'class="active"' : ''}>
                <a href="${ctx}/activities/adpush/${m.id}?BackURL=<es:BackURL/>">
                    <i class="icon-eye-open"></i>
                    查看
                </a>
            </li>
            <shiro:hasPermission name="activities:adpush:update">
            <li ${op eq '修改' ? 'class="active"' : ''}>
                <a href="${ctx}/activities/adpush/${m.id}/update?BackURL=<es:BackURL/>">
                    <i class="icon-edit"></i>
                    修改
                </a>
            </li>
            ...
            <shiro:hasPermission name="activities:adpush:delete">
            <li ${op eq '删除' ? 'class="active"' : ''}>
                <a href="${ctx}/activities/adpush/${m.id}/delete?BackURL=<es:BackURL/>">
        ...
        <%--<es:BackURL hiddenInput="true"/>--%>
            ...
            <div class="control-group">
             ...
             <div class="control-group">
              ...
              <div class="control-group">
                <form:label path="brand" cssClass="control-label">品牌  </form:label>
                <div class="controls">
                    <form:select path="brand">
               <c:forEach items="${brandList}" var ="p">
               <option value="" title="${p.ctsName}" cssClass="validate[required]">${p.ctsDesc}</option>
               </c:forEach>     
                    </form:select>
                </div>
            </div>
            ...
            <div class="control-group">
                <form:label path="sendMsg" cssClass="control-label">推送内容</form:label>
        ...
        <c:choose>
            ...
            <c:otherwise>
                ...
                $.validationEngineLanguage.allRules.ajaxNameCall= {
                    "url": "${ctx}/activities/adpush/validate",
                    //动态提取的数据。验证时一起发送
                    extraDataDynamic : ['#id'],
                    //验证失败时的消息
