...
<div data-table="table" class="panel">
    ...
    <ul class="nav nav-tabs">
        <li ${empty param['search.show_eq'] ? 'class="active"' : ''}>
            <a href="${ctx}/activities/tree/${parent.id}/children">
                <i class="icon-table"></i>
                [${parent.name}]子节点列表
            </a>
        </li>
        <li ${param['search.show_eq'] eq 'true' ? 'class="active"' : ''}>
            <a href="${ctx}/activities/tree/${parent.id}/children?search.show_eq=true">
                <i class="icon-table"></i>
                显示的
            </a>
        </li>
        <li ${param['search.show_eq'] eq 'false' ? 'class="active"' : ''}>
            <a href="${ctx}/activities/tree/${parent.id}/children?search.show_eq=false">
                <i class="icon-table"></i>
                隐藏的
            </a>
        </li>
        <li>
            <a href="${ctx}/activities/tree/${parent.id}/update">
    ...
    <div class="row-fluid tool ui-toolbar">
        <div class="span4">
            <div class="btn-group">
                <shiro:hasPermission name="activities:tree:create">
                <a class="btn btn-custom no-disabled" href="${ctx}/activities/tree/${parent.id}/appendChild?BackURL=<es:BackURL/>">
                    <i class="icon-file-alt"></i>
                    添加子节点
                </a>
                </shiro:hasPermission>
            <shiro:hasPermission name="activities:tree:update">
                <a id="updateTree" class="btn btn-custom">
                    <i class="icon-edit"></i>
                    修改
                </a>
                </shiro:hasPermission>
                <shiro:hasPermission name="activities:tree:delete">
...
<script type="text/javascript">
    $(function() {
        var tableId = "table";
        var urlPrefix = "${ctx}/activities/tree";
